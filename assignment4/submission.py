...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            csp.add_binary_factor(i, j,
                lambda col1, col2, i=i, j=j: 
                    (col1 != col2) and (abs(col1 - col2) != abs(i - j)))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch:
    ...
    def apply_arc_consistency(self, var) -> None:
        ...
        # helper function remove_inconsistent_values(var1,var2)
        #
        # enforce arc consistency from var1 to var2
        # 0. set boolean flag removed to False
        # 1. get the binary factor from var1 to var2 csp.binaryFactors[var1][var2]
        # 2. loop through the domain of var1
        #       if no value val2 for var2 in the domain of var2 satisfies the constraints
        #           remove val1 from domain of var1
        #           set removed to True
        #    return removed
        ...
        # set up a list initially containing var
        # while there are elements in the list:
        #   curr = pop the first element of the list
        #   for neighbor in neighbors of curr:
        #      if remove_inconsistent_values(neighbor,curr) then append neighbor to list
        # Note: domain update is done in helper function

        # BEGIN_YOUR_CODE (our solution is 8 lines of code, but don't worry if you deviate from this)
        queue = [var]
        while queue:
            curr = queue.pop(0)
            for neighbor in self.csp.binaryFactors[curr]:
    ...
    def add_quarter_constraints(self, csp: CSP) -> None:
        ...
        for request in self.profile.requests:
            if not request.quarters:
                continue
            curr_quarters = set(request.quarters)
            for quarter in self.profile.quarters:
                if quarter not in curr_quarters:
